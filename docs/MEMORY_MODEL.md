...
`array[T]` is a unique owner. Initializing another variable from it, or passing it to a by-value
parameter, transfers ownership and invalidates the original binding:
...
Use `&array[T]` or `&mut array[T]` for a non-consuming call. Tensor values are managed opaque
handles and use the public `Tensor[T]` methods. The validator reports moves, use-after-move, and
conflicts with active borrows before C generation.
