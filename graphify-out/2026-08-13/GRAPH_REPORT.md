...
## Corpus Check
- 79 files · ~97,979 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1329 nodes · 2730 edges · 76 communities (65 shown, 11 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 98 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fbd24063`
...
- ._generate_device_tensor_expression
- TypedModule
- ImportProcessor
- ocean-lang
- TypedScope
...
### Community 0 - "run"
Cohesion: 0.05
Nodes (52): Compatibility façade for the Phils Ocean C backend v0.2. Existing callers may…, run(), generate(), test_array_lowering_and_index_mutation(), test_tensor_lowering_shape_and_index_mutation(), test_tensor_numeric_methods_use_shape_and_stride_runtime(), test_tensor_rank_specialization_is_unbounded(), test_tensor_the_big_code() (+44 more)
...
Cohesion: 0.13
Nodes (6): Валидирует объявление указателя, Получает информацию о символе из текущего или родительских scope'ов, Проверяет выход за границы массивов/списков, Проверяет операции со строками, Пытается получить статическое значение из AST, Находит родительский узел (если есть)

### Community 31 - "._validate_argument"
Cohesion: 0.17
...
Cohesion: 0.17
Nodes (8): OrchestratorMixin, Generate C from the canonical semantic IR., Генерирует имя временной переменной, Генерирует код для узла графа, Compatibility entry point for callers with parser JSON., Return whether an AST value contains a direct ``@c_function(...)``., Lower the compatibility scope view after typed IR is established., Генерирует объявление глобальной переменной
...
Cohesion: 0.10
Nodes (11): Собирает информацию о всех символах в системе, Строит карту соответствия узлов исходным строкам, Возвращает отчет о проверке, Validate a typed module or legacy parser graph. ``TypedModule`` is the…, Validate the canonical semantic module before C lowering., Run the existing validation passes over a typed lowering view., Iterate semantic nodes in source/scope order., Return the compatibility graph used by legacy callers. (+3 more)
...
Cohesion: 0.21
Nodes (7): infer_literal_shape(), Split text only when not nested in (), [], {}, <> or strings., Recursive parser for Phils type expressions., Infer a rectangular shape from nested list literals. Returns ``None`` for…, split_top_level(), TypeParser, Typed intermediate representation for the Ocean compiler. The parser's…
...
- **Why does `CCodeGenerator` connect `CCodeGenerator` to `run`, `OopMixin`, `RuntimeError`, `TypesMixin`, `OwnershipMixin`, `generator.py`, `main.py`, `ArrayCodegenMixin`, `ListCodegenMixin`, `IndexingMixin`, `IoMixin`, `StatementsMixin`, `ScopeMixin`, `HelpersMixin`, `TupleCodegenMixin`, `CoreMixin`, `build_typed_ir`, `OrchestratorMixin`, `TensorCodegenMixin`, `NamingMixin`, `test_memory_safety.py`, `DictCodegenMixin`, `benchmark_main.py`, `ExpressionsMixin`, `ImportsMixin`?**
  _High betweenness centrality (0.268) - this node is a cross-community bridge._
- **Why does `JSONValidator` connect `JSONValidator` to `run`, `.validate_scope`, `.add_error`, `.check_undefined_methods`, `test_debug_validator.py`, `.get_type_from_ast`, `test_memory_safety.py`, `main.py`, `build_typed_ir`, `.get_symbol_info`, `TypedModule`, `benchmark_main.py`, `parser.py`, `._validate_argument`?**
  _High betweenness centrality (0.248) - this node is a cross-community bridge._
- **Why does `Parser` connect `Parser` to `run`, `modules/imports.py`, `CCodeGenerator`, `SymbolTable`, `.parse_function_arguments_to_ast`, `.parse_expression_to_ast`, `test_debug_validator.py`, `test_memory_safety.py`, `.calculate_indent_level`, `main.py`, `._parse_line_impl`, `build_typed_ir`, `.parse_function_call`, `.parse_type_annotation`, `benchmark_main.py`, `parser.py`?**
