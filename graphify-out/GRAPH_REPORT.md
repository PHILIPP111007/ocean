...
## Corpus Check
- 86 files · ~102,306 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1433 nodes · 3018 edges · 82 communities (70 shown, 12 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 101 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `14bfe06a`
...
Cohesion: 0.06
Nodes (18): Валидирует граф операций, Находит родительский scope для заданного уровня, Валидирует выражение (правая часть присваивания или инициализации), Валидирует удаление переменной, Валидирует унарную операцию, Валидирует составное присваивание, Валидирует объявление функции, Валидирует вызов функции с поддержкой AST аргументов (+10 more)
...
### Community 8 - "Parser"
Cohesion: 0.10
Nodes (17): Parser, Remove standalone triple-quoted blocks while preserving line count., Определяет текущий scope на основе отступа, Проверяет, является ли имя именем класса, Извлекает содержимое внутри скобок, учитывая вложенность, Удаляет дублирующиеся методы из классов, Собирает унаследованные методы для всех классов, Remove ``#`` comments without corrupting string literals. (+9 more)
...
### Community 33 - "TypedModule"
Cohesion: 0.12
Nodes (10): OrchestratorMixin, Генерирует имя временной переменной, Генерирует код для узла графа, Generate C from the canonical semantic IR., Lower typed scope views after semantic IR construction., Return whether an AST value contains a direct ``@c_function(...)``., Генерирует объявление глобальной переменной, Typed compilation unit exchanged by compiler passes. (+2 more)
...
Cohesion: 0.17
Nodes (11): infer_literal_shape(), Split text only when not nested in (), [], {}, <> or strings., Recursive parser for Phils type expressions., Infer a rectangular shape from nested list literals. Returns ``None`` for…, split_top_level(), TypeParser, _build_typed_module(), OwnershipEffect (+3 more)
...
Cohesion: 0.10
Nodes (12): CImportProcessor, ImportProcessor, Path, Обрабатывает импорт и возвращает содержимое импортируемого файла, Обрабатывает все импорты в коде и вставляет содержимое файлов, Просто регистрирует C импорт без парсинга, Yield repository std directories from the active source context., Reset all per-compilation parser state. A Parser instance can safely be reused… (+4 more)
...
## Knowledge Gaps
- **105 isolated node(s):** `ocean-lang`, `Project Structure & Module Organization`, `Build, Test, and Development Commands`, `Coding Style & Naming Conventions`, `Testing Guidelines` (+100 more)
...
- **Why does `Validator` connect `Validator` to `.validate_openmp_loop`, `TypedModule`, `.validate_graph`, `.add_error`, `.validate_static_method_call`, `Parser`, `.add_warning`, `test_memory_safety.py`, `main.py`, `debug.py`, `.get_symbol_info`, `benchmark_main.py`, `._validate_scopes`, `.check_undefined_methods`?**
  _High betweenness centrality (0.245) - this node is a cross-community bridge._
- **Why does `CCodeGenerator` connect `CCodeGenerator` to `run`, `OopMixin`, `CallsMixin`, `TypesMixin`, `OwnershipMixin`, `Parser`, `generator.py`, `main.py`, `ArrayCodegenMixin`, `ListCodegenMixin`, `IndexingMixin`, `IoMixin`, `StatementsMixin`, `ScopeMixin`, `HelpersMixin`, `TupleCodegenMixin`, `CoreMixin`, `TypedModule`, `TensorCodegenMixin`, `NamingMixin`, `test_memory_safety.py`, `benchmark_main.py`, `compiler.py`?**
  _High betweenness centrality (0.241) - this node is a cross-community bridge._
- **Why does `Parser` connect `Parser` to `run`, `compiler.py`, `.parse_complex_expression`, `.extract_dependencies_from_ast`, `SymbolTable`, `.parse_expression_to_ast`, `test_memory_safety.py`, `.calculate_indent_level`, `main.py`, `ImportProcessor`, `._parse_line_impl`, `debug.py`, `.parse_function_call`, `CCodeGenerator`, `.parse_type_annotation`, `benchmark_main.py`, `parser.py`?**
