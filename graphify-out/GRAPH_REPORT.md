...
## Corpus Check
- 79 files · ~95,639 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1324 nodes · 2744 edges · 76 communities (65 shown, 11 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 97 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3712425d`
...
### Community 0 - "run"
Cohesion: 0.06
Nodes (43): Compatibility façade for the Phils Ocean C backend v0.2. Existing callers may…, run(), generate(), test_array_lowering_and_index_mutation(), test_c_code_math(), test_c_code_pthread(), test_del(), test_dict() (+35 more)
...
Cohesion: 0.09
Nodes (14): Валидирует граф операций, Находит родительский scope для заданного уровня, Валидирует выражение (правая часть присваивания или инициализации), Валидирует унарную операцию, Валидирует составное присваивание, Валидирует вызов функции print, Валидирует оператор return, Валидирует узел цикла (+6 more)
...
Cohesion: 0.08
Nodes (17): Валидирует типы в узле, Валидирует типы в присваивании, Проверяет тип объявления по типизированному AST., Валидирует типы возвращаемых значений, Валидирует типы в условии while, Валидирует типы в условии if/elif, Валидирует типы в операциях, Валидирует запись через указатель (*p = значение) (+9 more)
...
Cohesion: 0.05
Nodes (24): Проверяет дублирование переменных в local_variables, Валидирует таблицу символов scope'а, Валидирует отдельный символ, Валидирует объявление функции, Проверяет, что функция имеет return если нужно, Проверяет циклы на корректность, Warn about locals unused across the complete nested graph., Проверяет, что все пути выполнения функции возвращают значение (+16 more)
...
### Community 23 - ".add_error"
Cohesion: 0.10
Nodes (10): Валидирует объявление переменной, Валидирует удаление переменной, Валидирует объявление указателя, Проверяет совместимость типов при присваивании, Добавляет ошибку с информацией о строке, Получает информацию о символе из текущего или родительских scope'ов, Проверяет корректное использование указателей, Проверяет выход за границы массивов/списков (+2 more)
...
### Community 55 - "._validate_argument"
Cohesion: 0.17
...
- **Why does `Validator` connect `Validator` to `.validate_openmp_loop`, `test_debug_validator.py`, `.validate_graph`, `test_typed_ir.py`, `.get_type_from_ast`, `.validate_scope`, `test_memory_safety.py`, `main.py`, `TypedModule`, `._validate_scopes`, `._validate_argument`, `.add_error`, `benchmark_main.py`, `parser.py`, `.check_undefined_methods`?**
  _High betweenness centrality (0.283) - this node is a cross-community bridge._
- **Why does `CCodeGenerator` connect `CCodeGenerator` to `run`, `OopMixin`, `CallsMixin`, `TypesMixin`, `OwnershipMixin`, `generator.py`, `main.py`, `ArrayCodegenMixin`, `ListCodegenMixin`, `IndexingMixin`, `IoMixin`, `StatementsMixin`, `ScopeMixin`, `HelpersMixin`, `TupleCodegenMixin`, `CoreMixin`, `OrchestratorMixin`, `TensorCodegenMixin`, `NamingMixin`, `test_memory_safety.py`, `benchmark_main.py`, `test_typed_ir.py`, `ExpressionsMixin`, `DictCodegenMixin`, `ImportsMixin`?**
  _High betweenness centrality (0.275) - this node is a cross-community bridge._
- **Why does `Parser` connect `Parser` to `run`, `test_debug_validator.py`, `test_typed_ir.py`, `SymbolTable`, `._parse_with_priorities`, `ImportProcessor`, `.parse_expression_to_ast`, `test_memory_safety.py`, `.calculate_indent_level`, `main.py`, `._parse_line_impl`, `.parse_function_call`, `CCodeGenerator`, `.parse_type_annotation`, `benchmark_main.py`, `parser.py`?**
